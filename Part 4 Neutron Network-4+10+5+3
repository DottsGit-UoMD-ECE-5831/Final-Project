# Part 4 - Neuron Network - Based Classification Model 4+10+5+3 ----------------------------------------------
...
max_iterations = 1000
mlp = MLPClassifier(hidden_layer_sizes=(10, 5), activation='relu', max_iter=1, warm_start=True, random_state=42)
...
for i in range(max_iterations):
    ...
    # For validation loss, fit again to get the validation loss
    validation_loss.append(mlp.loss_)

# Compute the norm of the loss function
loss_norm = np.linalg.norm(train_loss)
...
plt.plot(range(1, max_iterations + 1), train_loss, label='Training Loss')
...
print(f'Norm of the Loss Function: {loss_norm:.4f}')
