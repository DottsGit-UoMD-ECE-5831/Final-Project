# - #Part 2  without sklearn, Adding Confusion Matrix - -----------------------------
import numpy as np
...
class KMeans:
    ...
    def calculate_inertia(self):
        ...
        for i in range(self.X.shape[0]):
            centroid_idx = self._closest_centroid(self.X[i], self.centroids)
            inertia += euclidean_distance(self.X[i], self.centroids[centroid_idx]) ** 2
        return inertia
...
def plot_clusters_for_optimal_k(X, y, optimal_k, feature_indices):
    ...
    confusion = np.zeros((optimal_k, 3), dtype=int)
    for true_label in np.unique(y):
        for cluster in range(optimal_k):
            confusion[cluster, true_label] = np.sum((y == true_label) & (cluster_labels == cluster))
